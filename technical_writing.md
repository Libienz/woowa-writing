...
글로벌 트랜잭션 ID 기반 복제에서 레플리카 서버는 GTID를 통해 복제 진행 상황을 자동으로 관리하므로, 복제 위치를 수동으로 지정할 필요가 없습니다. GTID 기반으로 복제 위치를 자동으로 파악할 수 있다는 것은 장애 복구의 용이성으로 이어지며 데이터의 일관성을 향상시킵니다.
